...
{
	struct net *net =
		container_of(table->data, struct net, ipv4.sysctl_ip_prot_sock);
	int ret;
	int pports;
	int range[2];
	struct ctl_table tmp = {
...
static int proc_allowed_congestion_control(struct ctl_table *ctl, int write,
					   void __user *buffer, size_t *lenp,
					   loff_t *ppos)
{
...
{
	struct net *net =
		container_of(table->data, struct net, ipv4.sysctl_tcp_fastopen);
	...
	snprintf(tbl.data, tbl.maxlen, "%08x-%08x-%08x-%08x", user_key[0],
		 user_key[1], user_key[2], user_key[3]);
	ret = proc_dostring(&tbl, write, buffer, lenp, ppos);
...
bad_key:
	pr_debug("proc FO key set 0x%x-%x-%x-%x <- 0x%s: %u\n", user_key[0],
		 user_key[1], user_key[2], user_key[3], (char *)tbl.data, ret);
	kfree(tbl.data);
	return ret;
}
...
static void proc_configure_early_demux(int enabled, int protocol)
{
	...
	if (ipprot)
		ipprot->early_demux =
			enabled ? ipprot->early_demux_handler : NULL;

#if IS_ENABLED(CONFIG_IPV6)
	ip6prot = rcu_dereference(inet6_protos[protocol]);
	if (ip6prot)
		ip6prot->early_demux =
			enabled ? ip6prot->early_demux_handler : NULL;
#endif
	rcu_read_unlock();
}
...
static int proc_tfo_blackhole_detect_timeout(struct ctl_table *table, int write,
					     void __user *buffer, size_t *lenp,
					     loff_t *ppos)
{
	struct net *net =
...
static int proc_tcp_available_ulp(struct ctl_table *ctl, int write,
				  void __user *buffer, size_t *lenp,
				  loff_t *ppos)
{
	struct ctl_table tbl = {
