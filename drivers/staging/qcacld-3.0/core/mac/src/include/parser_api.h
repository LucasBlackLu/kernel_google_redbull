...
#define MAX_SIZE_OF_TRIPLETS_IN_COUNTRY_IE                                     \
	(COUNTRY_STRING_LENGTH * COUNTRY_INFO_MAX_CHANNEL)
...
QDF_STATUS
sir_convert_probe_req_frame2_struct(struct mac_context *mac, uint8_t *frame,
				    uint32_t len, tpSirProbeReq probe);
...
QDF_STATUS
sir_convert_assoc_req_frame2_struct(struct mac_context *mac, uint8_t *frame,
				    uint32_t len, tpSirAssocReq assoc);
/**
...
QDF_STATUS
sir_convert_reassoc_req_frame2_struct(struct mac_context *mac, uint8_t *frame,
				      uint32_t len, tpSirAssocReq assoc);

QDF_STATUS
sir_parse_beacon_ie(struct mac_context *mac, tpSirProbeRespBeacon pBeaconStruct,
		    uint8_t *pPayload, uint32_t payloadLength);

QDF_STATUS
sir_convert_beacon_frame2_struct(struct mac_context *mac, uint8_t *pBeaconFrame,
				 tpSirProbeRespBeacon pBeaconStruct);

QDF_STATUS
sir_convert_auth_frame2_struct(struct mac_context *mac, uint8_t *frame,
			       uint32_t len, tpSirMacAuthFrameBody auth);

QDF_STATUS
sir_convert_addts_rsp2_struct(struct mac_context *mac, uint8_t *frame,
			      uint32_t len, tSirAddtsRspInfo *addts);

QDF_STATUS
sir_convert_delts_req2_struct(struct mac_context *mac, uint8_t *frame,
			      uint32_t len, struct delts_req_info *delTs);
QDF_STATUS
...
void populate_dot11f_chan_switch_ann(struct mac_context *mac,
				     tDot11fIEChanSwitchAnn *pDot11f,
				     struct pe_session *pe_session);

void populate_dot_11_f_ext_chann_switch_ann(
	struct mac_context *mac_ptr, tDot11fIEext_chan_switch_ann *dot_11_ptr,
	struct pe_session *session_entry);

void populate_dot11f_vht_tx_power_env(struct mac_context *mac,
				      tDot11fIEvht_transmit_power_env *pDot11f,
				      enum phy_ch_width ch_width, uint8_t chan);
/* / Populate a tDot11fIEChannelSwitchWrapper */
void populate_dot11f_chan_switch_wrapper(struct mac_context *mac,
					 tDot11fIEChannelSwitchWrapper *pDot11f,
					 struct pe_session *pe_session);

/* / Populate a tDot11fIECountry */
QDF_STATUS
populate_dot11f_country(struct mac_context *mac, tDot11fIECountry *pDot11f,
			struct pe_session *pe_session);

/* Populated a populate_dot11f_ds_params */
QDF_STATUS
populate_dot11f_ds_params(struct mac_context *mac, tDot11fIEDSParams *pDot11f,
			  uint8_t channel);

/* / Populated a tDot11fIEEDCAParamSet */
void populate_dot11f_edca_param_set(struct mac_context *mac,
				    tDot11fIEEDCAParamSet *pDot11f,
				    struct pe_session *pe_session);

QDF_STATUS
populate_dot11f_erp_info(struct mac_context *mac, tDot11fIEERPInfo *pDot11f,
			 struct pe_session *pe_session);
...
QDF_STATUS
populate_dot11f_ext_supp_rates1(struct mac_context *mac, uint8_t nChannelNum,
				tDot11fIEExtSuppRates *pDot11f);

QDF_STATUS
populate_dot11f_ht_caps(struct mac_context *mac, struct pe_session *pe_session,
			tDot11fIEHTCaps *pDot11f);

QDF_STATUS
populate_dot11f_ht_info(struct mac_context *mac, tDot11fIEHTInfo *pDot11f,
			struct pe_session *pe_session);
...
void populate_dot11f_power_caps(struct mac_context *mac,
				tDot11fIEPowerCaps *pCaps, uint8_t nAssocType,
				struct pe_session *pe_session);
...
void populate_dot11f_qos_caps_station(struct mac_context *mac,
				      struct pe_session *session,
				      tDot11fIEQOSCapsStation *pDot11f);

QDF_STATUS
populate_dot11f_rsn(struct mac_context *mac, tpSirRSNie pRsnIe,
		    tDot11fIERSN *pDot11f);

QDF_STATUS
populate_dot11f_rsn_opaque(struct mac_context *mac, tpSirRSNie pRsnIe,
			   tDot11fIERSNOpaque *pDot11f);

#if defined(FEATURE_WLAN_WAPI)

QDF_STATUS
populate_dot11f_wapi(struct mac_context *mac, tpSirRSNie pRsnIe,
...
void populate_dot11f_ssid(struct mac_context *mac, tSirMacSSid *pInternal,
...
void populate_dot11f_schedule(tSirMacScheduleIE *pSchedule,
			      tDot11fIESchedule *pDot11f);

void populate_dot11f_supp_channels(struct mac_context *mac,
...
QDF_STATUS
populate_dot11f_supp_rates(struct mac_context *mac, uint8_t nChannelNum,
...
void populate_dot11f_wmm(struct mac_context *mac, tDot11fIEWMMInfoAp *pInfo,
			 tDot11fIEWMMParams *pParams, tDot11fIEWMMCaps *pCaps,
			 struct pe_session *pe_session);
...
void populate_dot11_tsrsie(struct mac_context *mac, struct ese_tsrs_ie *pOld,
...
void populate_dot11f_re_assoc_tspec(struct mac_context *mac,
				    ...
				    struct pe_session *pe_session);
QDF_STATUS
sir_beacon_ie_ese_bcn_report(struct mac_context *mac, uint8_t *pPayload,
			     const uint32_t payloadLength, uint8_t **outIeBuf,
			     uint32_t *pOutIeLen);
...
void populate_dot11f_wmm_info_station_per_session(
	struct mac_context *mac, struct pe_session *pe_session,
...
QDF_STATUS
populate_dot11f_wpa(struct mac_context *mac, tpSirRSNie pRsnIe,
		    tDot11fIEWPA *pDot11f);

QDF_STATUS
populate_dot11f_wpa_opaque(struct mac_context *mac, tpSirRSNie pRsnIe,
...
QDF_STATUS
populate_dot11f_tclas(struct mac_context *mac, tSirTclasInfo *pOld,
		      tDot11fIETCLAS *pDot11f);

QDF_STATUS
populate_dot11f_wmmtclas(struct mac_context *mac, tSirTclasInfo *pOld,
...
de_populate_dot11f_wsc_registrar_info_in_probe_res(
	struct mac_context *mac, tDot11fIEWscProbeRes *pDot11f);
...
void populate_fils_ft_info(struct mac_context *mac, tDot11fIEFTInfo *ft_info,
			   struct pe_session *pe_session);
#else
static inline void populate_fils_ft_info(struct mac_context *mac,
					 tDot11fIEFTInfo *ft_info,
					 struct pe_session *pe_session)
{
}
#endif
...
void populate_dot11f_qcn_ie(struct mac_context *mac, tDot11fIEqcn_ie *qcn_ie,
			    uint8_t attr_id);
...
void populate_dot11f_avoid_channel_ie(struct mac_context *mac_ctx,
				      tDot11fIEQComVendorIE *dot11f,
				      struct pe_session *session_entry);
#endif /* FEATURE_AP_MCC_CH_AVOIDANCE */

QDF_STATUS
populate_dot11f_timing_advert_frame(struct mac_context *mac,
				    tDot11fTimingAdvertisementFrame *frame);
void populate_dot11_supp_operating_classes(
	struct mac_context *mac_ptr, tDot11fIESuppOperatingClasses *dot_11_ptr,
	struct pe_session *session_entry);

QDF_STATUS
sir_validate_and_rectify_ies(struct mac_context *mac_ctx, uint8_t *mgmt_frame,
			     uint32_t frame_bytes, uint32_t *missing_rsn_bytes);
/**
...
QDF_STATUS populate_dot11f_he_caps(struct mac_context *, struct pe_session *,
				   tDot11fIEhe_cap *);
QDF_STATUS populate_dot11f_he_operation(struct mac_context *,
					struct pe_session *, tDot11fIEhe_op *);
#ifdef WLAN_FEATURE_11AX_BSS_COLOR
QDF_STATUS
...
#else
static inline QDF_STATUS
populate_dot11f_twt_extended_caps(struct mac_context *mac_ctx,
				  struct pe_session *pe_session,
				  tDot11fIEExtCap *dot11f)
{
	return QDF_STATUS_SUCCESS;
