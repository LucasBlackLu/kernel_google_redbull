 ...
 */
static int mxc_clkdivs[] = { 0,	 3,  4,	  6,   8,   12,	 16,  24,  32,	48,
			     64, 96, 128, 192, 256, 384, 512, 768, 1024 };

/* MX21, MX27 */
static unsigned int spi_imx_clkdiv_1(unsigned int fin, unsigned int fspi,
				     unsigned int max, unsigned int *fres)
{
	int i;
...
static void mx53_ecspi_rx_slave(struct spi_imx_data *spi_imx)
{
	...
	if (spi_imx->rx_buf) {
		...
		memcpy(spi_imx->rx_buf, ((u8 *)&val) + sizeof(val) - n_bytes,
		       n_bytes);
...
static struct spi_imx_devtype_data imx27_cspi_devtype_data = {
	...
	.intctrl = mx21_intctrl, .config = mx21_config,
	.trigger = mx21_trigger, .rx_available = mx21_rx_available,
	.reset = mx21_reset,	 .fifo_size = 8,
	.has_dmamode = false,	 .dynamic_burst = false,
	.has_slavemode = false,	 .devtype = IMX27_CSPI,
};
...
static struct spi_imx_devtype_data imx35_cspi_devtype_data = {
	...
	.intctrl = mx31_intctrl, .config = mx31_config,
	.trigger = mx31_trigger, .rx_available = mx31_rx_available,
	.reset = mx31_reset,	 .fifo_size = 8,
	.has_dmamode = true,	 .dynamic_burst = false,
	.has_slavemode = false,	 .devtype = IMX35_CSPI,
};
...
static void spi_imx_push(struct spi_imx_data *spi_imx)
{
	...
	while (spi_imx->txfifo < spi_imx->devtype_data->fifo_size) {
		...
		if (spi_imx->dynamic_burst &&
		    spi_imx->txfifo >=
			    DIV_ROUND_UP(spi_imx->remainder, fifo_words))
			break;
		spi_imx->tx(spi_imx);
		spi_imx->txfifo++;
	}
...
static int spi_imx_setupxfer(struct spi_device *spi, struct spi_transfer *t)
{
	...
	if (spi_imx->devtype_data->dynamic_burst && !spi_imx->slave_mode &&
	    (spi_imx->bits_per_word == 8 || spi_imx->bits_per_word == 16 ||
				...
				struct spi_transfer *transfer)
{
	 ...
	 * before TX.
	 */
	desc_rx = dmaengine_prep_slave_sg(master->dma_rx, rx->sgl, rx->nents,
					  DMA_DEV_TO_MEM,
	...
	desc_tx = dmaengine_prep_slave_sg(master->dma_tx, tx->sgl, tx->nents,
					  DMA_MEM_TO_DEV,
					  DMA_PREP_INTERRUPT | DMA_CTRL_ACK);
	if (!desc_tx) {
...
static int spi_imx_prepare_message(struct spi_master *master,
				   struct spi_message *msg)
{
	struct spi_imx_data *spi_imx = spi_master_get_devdata(master);
	int ret;
...
static int spi_imx_unprepare_message(struct spi_master *master,
				     struct spi_message *msg)
{
	struct spi_imx_data *spi_imx = spi_master_get_devdata(master);
...
static int spi_imx_probe(struct platform_device *pdev)
{
	...
	if (mxc_platform_info) {
		...
		if (mxc_platform_info->chipselect) {
			...
			for (i = 0; i < master->num_chipselect; i++)
				master->cs_gpios[i] =
					mxc_platform_info->chipselect[i];
		}
	} else {
		u32 num_cs;
	...
	spi_imx->bitbang.master->mode_bits =
		SPI_CPOL | SPI_CPHA | SPI_CS_HIGH | SPI_NO_CS;
