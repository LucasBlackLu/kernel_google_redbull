		     ...
		     struct node_info *dni, block_t blkaddr, unsigned int *nofs)
{
	...
	max_addrs =
		IS_INODE(node_page) ? DEF_ADDRS_PER_INODE : DEF_ADDRS_PER_BLOCK;
	if (ofs_in_node >= max_addrs) {
		f2fs_err(
			sbi,
			"Inconsistent ofs_in_node:%u in summary, ino:%u, nid:%u, max:%u",
...
retry:
	...
	if (gc_control->init_gc_type == FG_GC ||
	    !has_not_enough_free_secs(sbi, (gc_type == FG_GC) ? sec_freed : 0,
				      0)) {
